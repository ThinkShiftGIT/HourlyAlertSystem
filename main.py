...
try:
    ...
    BS4_AVAILABLE = False
    logging.warning("BeautifulSoup not available. Article scraping disabled.")
...
def analyze_sentiment(text: str) -> float:
    ...
    return 0.5 if pos_count > neg_count else -0.5 if neg_count > pos_count else 0.0

@retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=4, max=10))
def send_telegram_alert(message, chat_ids=CHAT_IDS):
    logger.info(f"Sending alert to: {chat_ids}")
    for chat_id in chat_ids:
        try:
            ...
            if response.status_code != 200:
                ...
                logger.info(f"Telegram sent: {response.text}")
            time.sleep(1)
...
def main():
    ...
    scheduler.add_job(lambda: logger.info("[Scheduler] News scan triggered") or fetch_and_analyze_news(), 'interval', minutes=SCAN_INTERVAL_MINUTES)
    scheduler.start()
